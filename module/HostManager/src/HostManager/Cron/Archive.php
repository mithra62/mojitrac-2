...
class Archive extends BaseCron
{
	...
	public function run()
	{
		...
		foreach($accounts AS $site)
		{
			...
			$site_settings = $settings->reset()->getSettings(array('account_id' => $site['id']));
			$last_ran = $site_settings['_task_auto_archive_last_ran'];
			if($last_ran == '')
			{
				$this->console->writeLine('Skipping: '.$site['slug'].' since never ran before...');
				$settings->updateSettings(array('_task_auto_archive_last_ran' => time()), array('account_id' => $site['id']));
				continue;
			}
