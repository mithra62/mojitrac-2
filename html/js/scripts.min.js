...
(function($){$.facebox=function(data,klass){$.facebox.loading()
	...
	$.extend($.facebox,{settings:{opacity:0,overlay:true,loadingImage:'http://static.mojitrac.com/images/loading.gif',closeImage:'http://static.mojitrac.com/images/closelabel.gif',imageTypes:['png','jpg','jpeg','gif'],faceboxHtml:'\
...
Progressbar
*/
(function($){$.extend({progressBar:new function(){this.defaults={steps:20,step_duration:20,max:100,showText:true,textFormat:'percentage',width:120,height:12,callback:null,boxImage:'http://static.mojitrac.com/images/progressbar/progressbar.gif',barImage:{0:'/images/progressbar/progressbg_red.gif',30:'http://static.mojitrac.com/images/progressbar/progressbg_orange.gif',70:'http://static.mojitrac.com/images/progressbar/progressbg_green.gif'},running_value:0,value:0,image:null};this.construct=function(arg1,arg2){var argvalue=null;var argconfig=null;if(arg1!=null){if(!isNaN(arg1)){argvalue=arg1;if(arg2!=null){argconfig=arg2;}}else{argconfig=arg1;}}
...
(function($){$.fn.shortPass='Too short';$.fn.badPass='Weak';$.fn.goodPass='Good';$.fn.strongPass='Strong';$.fn.samePassword='Email and Password identical.';$.fn.resultStyle="";$.fn.passStrength=function(options){var defaults={shortPass:"shortPass",badPass:"badPass",goodPass:"goodPass",strongPass:"strongPass",baseStyle:"testresult",userid:"",enableUcheck:0,messageloc:1};var opts=$.extend(defaults,options);return this.each(function(){var obj=$(this);$(obj).unbind().keyup(function()
...
var settings={cls:'jquery-checkbox',empty:'http://static.mojitrac.com/images/empty.png'};settings=$.extend(settings,options||{});var addEvents=function(object)
...
$.fn.dragCheck=function(selector){if(selector===false)
return this.find('*').andSelf().add(document).unbind('.dc').removeClass('dc-selected').filter(':has(:checkbox)').css({MozUserSelect:'',cursor:''});else
return this.each(function(){var mdown=null;$(this).find(selector||'> *').filter(':has(:checkbox)').each(function(){if($(this).find(':checkbox:checked').length)
$(this).addClass('dc-selected');}).bind('mouseover.dc',function(){if(mdown!=null){$(this).find(':checkbox')[0].checked=mdown;$(this).toggleClass('dc-selected',mdown);}}).bind('mousedown.dc',function(e){var t=e.target;if(!$(t).is(':checkbox'))
t=$(this).find(':checkbox')[0];t.checked=!t.checked;mdown=t.checked;$(this).toggleClass('dc-selected',mdown);}).bind('selectstart.dc',function(){return false;}).css({MozUserSelect:'none',cursor:'default'}).find(':checkbox').bind('click.dc',function(){return false;});$(document).bind('mouseup.dc',function(){mdown=null;});});};$("#mytable").dragCheck('td:not(.nono)');$("#mylist").dragCheck('li:not(.nono)');$("#clear").click(function(){$("#mytable").dragCheck(false);});$("#set").click(function(){$("#mytable").dragCheck('td:not(.nono)');});
